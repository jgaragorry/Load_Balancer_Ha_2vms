...
az network lb probe create --resource-group $RG --lb-name $LB --name $PROBE --protocol Http --port 80 --path "/" --probe-threshold 1 --interval 15
...
echo "🔁 Agregando NICs al backend del LB..."
for vm in $VM1 $VM2; do
  while true; do
    NIC_ID=$(az vm show --resource-group $RG --name $vm --query "networkProfile.networkInterfaces[0].id" -o tsv 2>/dev/null) && break
    echo "⏳ Esperando NIC de $vm..."
    sleep 5
  done
  NIC_NAME=$(basename $NIC_ID)
  az network nic ip-config address-pool add --resource-group $RG --nic-name $NIC_NAME --ip-config-name ipconfig1 --lb-name $LB --address-pool $BACKEND
done
